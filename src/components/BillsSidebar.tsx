...
// Enhanced Reporting Hub Component with dark theme
...
<div className="mb-4">
<h2 className="text-lg font-bold mb-1 flex items-center text-red-400">
...
className={`w-full p-3 rounded-lg border border-gray-700 hover:border-red-500 transition-all duration-200 text-left group bg-gray-900 hover:bg-gray-800`}
>
...
<div>
<h3 className="font-semibold text-xs text-gray-200">{category.title}</h3>
...
category.urgency === 'urgent' ? 'bg-red-900 text-white' :
category.urgency === 'high-risk' ? 'bg-red-800 text-white' :
category.urgency === 'moderate' ? 'bg-red-700 text-white' :
...
<h3 className="font-semibold text-xs mb-1 text-red-400">🔒 Security Notice</h3>
...
size="sm"
onClick={onClose}
className="text-xs w-full bg-gray-900 border-red-800 text-red-400 hover:bg-red-900/30"
>
Close Reporting Hub
</Button>
</div>
...
<div className="absolute bottom-0 left-0 right-0 bg-gray-900 rounded-t-2xl shadow-2xl max-h-[85vh] overflow-hidden border-t border-red-800">
<div className="flex justify-between items-center p-3 border-b border-red-800">
<div className="flex items-center">
<FileText className="h-5 w-5 mr-2 text-red-400" />
<span className="font-bold text-red-400">CEKA Bill Tracker</span>
</div>
<Button variant="ghost" size="icon" onClick={onClose} className="text-red-400 hover:bg-red-900/30">
<X className="h-5 w-5" />
</Button>
</div>
<div className="overflow-y-auto max-h-[75vh] p-4 bg-gray-900">
{children}
</div>
</div>
</div>
...
// Enhanced BillsSidebar with dark theme
...
// Handle window resize to close sidebar on mobile-to-desktop transition
useEffect(() => {
const handleResize = () => {
...
return (
<div className="space-y-3">
<h3 className="font-bold text-lg mb-2 text-gray-200">Related Bills</h3>
{relatedBills.map((bill, index) => (
<a
key={index}
href={bill.path}
className="block p-3 rounded-lg border border-gray-700 hover:border-red-500 hover:bg-gray-800 transition-colors"
>
...
<span className="text-xs text-gray-400">{bill.year}</span>
<span className={`text-xs px-1.5 py-0.5 rounded-full ${
bill.status === 'Active' ? 'bg-green-900 text-green-300' :
bill.status === 'Pending' ? 'bg-yellow-900 text-yellow-300' :
...
<h3 className="font-bold text-lg text-gray-200">Suggest a Bill</h3>
<Textarea
placeholder="Suggest a bill for us to cover..."
value={suggestion}
onChange={(e) => setSuggestion(e.target.value)}
className="min-h-[120px] bg-gray-800 border-gray-700 text-gray-200"
/>
...
return (
<div className="space-y-4">
<h3 className="font-bold text-lg text-gray-200">Support CEKA</h3>
<Textarea
...
<Card className="w-64 shadow-xl border border-red-800 bg-gray-900">
<CardHeader className="pb-2">
<CardTitle className="flex items-center justify-between text-sm text-red-400">
<div className="flex items-center gap-2">
...
{isExpanded && (
<CardContent className="pt-0 max-h-[70vh] overflow-y-auto bg-gray-900">
{showReportHub ? (
...
key={index}
href={bill.path}
className="block p-2 rounded-lg border border-gray-800 hover:border-red-500 hover:bg-gray-800 transition-colors group"
>
<div className="flex items-start justify-between">
<div className="flex-1 min-w-0">
...
<ExternalLink className="h-3 w-3 text-gray-500 mt-1 group-hover:text-red-400 transition-colors" />
</div>
</a>
))}
</div>
...
variant="outline"
size="sm"
className="w-full text-xs border-red-800 text-red-400 hover:bg-red-900/30"
...
{showSuggestionForm && (
<div className="mt-4 p-4 bg-gray-800 rounded-lg border border-red-800">
<h3 className="font-semibold text-xs mb-2 text-gray-200">Suggest a Bill</h3>
<Textarea
placeholder="Suggest a bill for us to cover..."
value={suggestion}
onChange={(e) => setSuggestion(e.target.value)}
className="mb-2 text-xs min-h-[60px] bg-gray-900 border-gray-700 text-gray-200"
/>
...
<h3 className="font-semibold text-xs mb-2 text-red-400">Support CEKA</h3>
<Textarea
placeholder="How would you like to support CEKA?"
value={supportMessage}
onChange={(e) => setSupportMessage(e.target.value)}
...
<div className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-red-800 flex justify-around items-center p-2 lg:hidden z-40">
<Button
variant="ghost"
className="flex flex-col items-center text-xs px-2 text-gray-400 hover:text-red-400"
